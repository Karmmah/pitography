...
import PIL, io, traceback, time, subprocess
...
def main(picam2, disp, preview_config, capture_config):
    ...
    while True:
        ...
        check_energy_saver_activation()

        # show menu
        #main menu
        if current_menu_index == 0 and input_key == key1_pin: #enter menu
            current_menu_index = main_menu_index
            #continue

        if current_menu_index == main_menu_index:
            main_menu_screen_draw.line((106,56,88,56), width=2, fill=ps.red if current_capture_mode == still_capture_index else ps.white)
            main_menu_screen_draw.line((46,83,82,83), width=2, fill=ps.red if current_capture_mode == timelapse_capture_index else ps.white)
            disp.LCD_ShowImage(screens.main_menu_screen, 0, 0)
            time.sleep(0.4) #delay to not immediately return from menu if button is not yet released
            input_key = 0
            while input_key == 0:
                input_key = check_input()
                time.sleep(0.1)
        ...
        if energy_saving_flag == True and i%5 != 0:
            ...
            continue
        if i%5 == 0:
            overlay_draw.rectangle((0,0,LCD_1in44.LCD_WIDTH,LCD_1in44.LCD_HEIGHT), fill=ps.black)
            ...
            if current_capture_mode == timelapse_capture_index:
                ...
                overlay_draw.text( (1,1), " Photo", fill=ps.white ) #drop shadow like to make it look nicer
                overlay_draw.text( (0,0), " Photo", fill=ps.white )
            # add capture parameters to overlay
            metadata = picam2.capture_metadata()
            overlay_draw.text((3,27), "prev", fill=ps.white)
            overlay_draw.text((3,58), "-", fill=ps.white)
            overlay_draw.text((3,87), "menu", fill=ps.white)
            overlay_draw.text((90,18), f"ag\n{str(round(metadata["AnalogueGain"],1))}", fill=ps.white)
            overlay_draw.text((90,48), f"dg\n{str(round(metadata["DigitalGain"],1))}", fill=ps.white)
            overlay_draw.text((90,78), f"e\n1/{str(int((metadata["ExposureTime"]/1000000)**(-1)))}", fill=ps.white)
            with open("/sys/class/thermal/thermal_zone0/temp") as f:
                temp = int(int(f.read().rstrip("\n"))/1000)
            overlay_draw.text((3, 114), f"t {str(temp)}", fill=ps.white)
            connection = subprocess.check_output("hostname -I", shell=True, text=True)[:13]
            overlay_draw.text((28,114), f'|{connection if len(connection) >= 4 and connection[3] == "." else "no connection"}', fill=ps.white)
