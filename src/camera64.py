...
def run(picam2, disp, preview_config, capture_config):
    ...
    while True: #program loop
        ...
        if time.time() - last_input_time > 30: #enable energy saver after 30 s of no input
...
#        print("[!] DEBUG input key:{inputKey} button hold:{buttonHoldFlag}")
        ...
        if currentCaptureMode == still_capture_index and inputKey == press_pin:
            ...
            last_input_time = time.time() #avoid energy saving from inactivity during capture
